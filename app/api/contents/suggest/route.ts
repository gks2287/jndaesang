...
type SearchedContent = {
  ...
  sourceUrl?: string;  // 웹서칭에서 찾은 콘텐츠 원문 출처 링크 (있으면)
};
...
// 응답 content에서 web_search 도구 결과의 실제 원문 URL 목록을 추출 (중복 제거, 등장 순서 유지).
// AI가 sourceUrl을 못 채웠을 때 콘텐츠 출처 링크 폴백으로 사용한다.
function extractSearchResultUrls(content: unknown[]): string[] {
  const urls: string[] = [];
  for (const block of content) {
    const b = block as { type?: string; content?: unknown };
    if (b?.type !== 'web_search_tool_result' || !Array.isArray(b.content)) continue;
    for (const r of b.content as Array<{ type?: string; url?: unknown }>) {
      if (r?.type === 'web_search_result' && isValidImageUrl(r.url)) {
        const u = r.url.trim();
...
[
  {
    ...
    for (let attempt = 1; attempt <= 2 && !searched; attempt += 1) {
      ...
      if (searched) {
        searchResultUrls = extractSearchResultUrls(response.content);
        console.log(`[contents/suggest] 파싱 성공: ${searched.length}개, 웹서칭 출처 ${searchResultUrls.length}건 (시도 ${attempt}/2)`);
      } else {
    ...
    const contents: ContentPoolItem[] = searched
      ...
      .map((c, i) => {
        ...
        // 출처 링크: AI가 제시한 sourceUrl 우선(유효한 http URL만), 없으면 웹서칭 결과 URL을 순서대로 폴백
        const sourceUrl = isValidImageUrl(c.sourceUrl)
          ? c.sourceUrl.trim()
          : (searchResultUrls[i] ?? searchResultUrls[0] ?? undefined);
        return {
