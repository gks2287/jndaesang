...
function buildPrompt(companyName: string, groups: GroupInput[], rawBlock: string): string {
...
${rawBlock ? '- 다면진단 보고서 원문이 제공된 경우, 유형 요약보다 원문에 서술된 실제 진단 내용을 우선 근거로 사용하세요.' : ''}
...
export async function POST(req: NextRequest) {
  try {
    const { companyName, groups, companyId, infoYear } = (await req.json()) as {
      companyName?: string;
      groups?: GroupInput[];
    ...
    const raw = await callClaude(buildPrompt(companyName?.trim() || '대상 기업', validGroups, rawBlock), SYSTEM_PROMPT);
